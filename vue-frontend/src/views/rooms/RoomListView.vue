<template>
  ...
  >
    ...
    <router-link class="mb-4 btn btn-info" to="/rooms/addRoom"
      >Add Room</router-link
    >
    <div class="shadow-sm">
      <table
        class="table table-bordered table-hover table-striped align-middle text-center bg-light"
      >
          ...
          <template v-for="tempRoom in paginatedRooms" :key="tempRoom.roomId">
            <tr>
              ...
              <td>
                <div class="dropdown">
                  <button
                    class="btn btn-info btn-sm dropdown-toggle px-3"
                    type="button"
                  ...
                  <ul class="dropdown-menu dropdown-menu-end">
                    <li>
                      <a
                        class="dropdown-item"
                        @click="
                          $router.push(`/rooms/roomDetails/${tempRoom.roomId}`)
                        "
                        >Details</a
                      >
                    </li>
                    <li>
                      <a
                        class="dropdown-item"
                        @click="
                          $router.push(`/rooms/updateRoom/${tempRoom.roomId}`)
                        "
                        >Update</a
                      >
                    </li>
                    <li>
                      <a
                        ...
                        @click="deleteRoom(`${tempRoom.roomId}`)"
                        >Delete</a
                      >
                    </li>
                  </ul>
...
export default defineComponent({
  ...
  methods: {
    ...
    listAllRooms(): Promise<any> {
      return new Promise(() => {
        this.roomService.collectAllRooms().then((response) => {
    ...
    async deleteRoom(roomId: any) {
      ...
      try {
        ...
        // Prva mera - splice za direktnu mutaciju niza
        const index = this.roomList.findIndex((room) => room.roomId === roomId);
        if (index !== -1) {
          this.roomList.splice(index, 1);
        }

        // Druga mera - ponovno učitavanje podataka
        await this.listAllRooms();

        // Treća mera - ručno ažuriranje paginacije
        this.totalPages = Math.max(
          1,
          Math.ceil(this.roomList.length / this.pageSize)
        );
        this.currentPage = Math.min(this.currentPage, this.totalPages);

        // Četvrta mera - osvežavanje view-a
        this.$forceUpdate();
      } catch (error) {
  ...
  created() {
    Promise.all([this.listAllRooms(), this.listAllCategories()]).catch(
      (error) => {
